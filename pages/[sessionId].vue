...
const events = useStorage<Record<string, any>[]>(`events-${sessionId.value}`, [])
const selectedEvent = useStorage<Record<string, any> | null>(`selectedEvent-${sessionId.value}`, null)

const { data, error, status, event: eventName } = useEventSource(
  `/api/all?sessionId=${sessionId.value}`,
  ['handshake', 'request', 'update', 'botError'] as const,
)
      ...
      <div class="page h-full w-full">
        <template v-if="!error">
          <div class="sidebar overflow-auto bg-mantle w-2/12 flex flex-col h-[calc(100vh-40px)] fixed top-10 z-10">
            <div
              v-for="event in events" :key="event?.meta?.id" class="flex flex-col data-[selected=true]:bg-surface0 p-2"
